    ...
    spawn(xPos, yPos, sides, radius, color) {
        ...
        mob[i] = Matter.Bodies.polygon(xPos, yPos, sides, radius, {
            ...
            death() {
                ...
                // Award currency on mob death
                if (typeof window.progression !== 'undefined' && typeof window.currencyDisplay !== 'undefined') {
                    const earned = window.progression.awardCurrency(this.tier || 0, this.isBoss || false);
                    window.currencyDisplay.showEarning(earned, this.position.x, this.position.y);
                }
