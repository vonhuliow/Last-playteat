...
// Gacha and Ying-Yang field controls
window.addEventListener("keydown", (event) => {
    if (event.key === "f" || event.key === "F") {
        if(m.fieldMode !== null && m.fieldUpgrades[m.fieldMode]) {
            const field = m.fieldUpgrades[m.fieldMode];
            if(field.name === "gacha field" && field.spin) {
                field.spin();
            }
        }
    }
    ...
    if (event.key === "g" || event.key === "G") {
        if(m.fieldMode !== null && m.fieldUpgrades[m.fieldMode]) {
            const field = m.fieldUpgrades[m.fieldMode];
            if(field.name === "ying yang field" && field.toggleMode) {
                field.toggleMode();
            }
        }
    }
